...
@$cp22 = $res[$i]['fotos'];
// Avisos financeiros: valores carregados para preservar configuracao ao editar a conta.
@$avisoVencimento = (int) $res[$i]['aviso_vencimento'];
@$avisoBaixa = (int) $res[$i]['aviso_baixa'];
@$avisoForma = $res[$i]['aviso_forma'] ?: 'email';
@$avisoDias = (int) ($res[$i]['aviso_dias'] ?: 2);

$cp22f =  implode('/', array_reverse(explode('-', $cp22)));
...
if($cp13 == 'Paga'){
	...
	$ocutar = '';
		...
		<td>
		<a href="#" onclick="editar('{$id}', '{$cp1}', '{$cp2}', '{$cp3}', '{$cp4}', '{$cp5}', '{$cp17}', '{$cp6}', '{$cp7}', '{$cp8}', '{$cp9}','{$cp14}', '{$cp15}', '{$cp16}', '{$nome_cliente}', '{$avisoVencimento}', '{$avisoBaixa}', '{$avisoForma}', '{$avisoDias}')" title="Editar Registro">	<i class="bi bi-pencil-square text-primary {$ocutar}"></i> </a>
		...
		<a href="#" onclick="fotos('{$id}')" title="Arquivos">	<i class="bi bi-upload text-success mx-1 {$ocutar}"></i> </a>
		<a href="#" onclick="mostrarComprovantes('{$id}')" title="Comprovante">	<i class="bi bi-card-checklist text-success mx-1 {$ocutar}"></i> </a>
	    </td>
		</tr>
	HTML;
	} 
...
function editar(id, cp1, cp2, cp3, cp4, cp5, cp17, cp6, cp7, cp8, cp9, cp14, cp15, cp16, nome, avisoVencimento, avisoBaixa, avisoForma, avisoDias){
	...
	$('#<?=$campo15?>').val(cp15);
	$('#<?=$campo16?>').val(cp16);
	// Avisos financeiros: restaura checks e canal ao abrir edicao.
	$('#aviso_vencimento').prop('checked', avisoVencimento == '1');
	$('#aviso_baixa').prop('checked', avisoBaixa == '1');
	$('#aviso_forma').val(avisoForma || 'email');
	$('#aviso_dias').val(avisoDias || 2);
	...
	if(usuario != 'Administrador'){
		...
		document.getElementById("<?=$campo4?>").readOnly = true;
	}
	
	//var plano = cp5;

	$('#plano_conta').val(cp5)
	$('#cat_despesas').val(cp17)
	listarDespesas(cp17)
	//('#<?=$campo5?>').val(cp5)
...
function limparCampos(){
	...
	$('#<?=$campo15?>').val('');
	$('#<?=$campo16?>').val('');
	// Avisos financeiros: novos cadastros saem com vencimento ativo e antecedencia padrao de 2 dias.
	$('#aviso_vencimento').prop('checked', true);
	$('#aviso_baixa').prop('checked', false);
	$('#aviso_forma').val('email');
	$('#aviso_dias').val(2);
...
function baixar(id, descricao, valor, saida,  valorj, desconto, multa, juros){
	...
	function mostrarComprovantes(id){

$.ajax({
   url: pag + "/listar_comprovantes",
   method: 'POST',
   data: {id},
   dataType: "html",
	  
   success:function(result){
	   $("#listar_comprovantes").html(result);
   }
   
});
console.log(id);
var myModal = new bootstrap.Modal(document.getElementById('modalComprovantes'), {		});
myModal.show();
$('#mensagem').text('');
}
