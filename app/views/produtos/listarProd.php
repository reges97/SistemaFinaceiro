...
echo <<<HTML
	...
	<td>{$nome_forn}</td>	
	<td>{$nome_cat}</td>	
	<td><img src="config/img/{$pagina}/{$cp9}" width="40px"></td>									
	<td>
	...
	<a class="mx-1" href="#" onclick="mostrarDados('{$id}', '{$cp1}', '{$cp2}', '{$cp3}', '{$cp4}', '{$cp5}', '{$cp6}', '{$nome_forn}', '{$nome_cat}', '{$cp9}', '{$cp10}')" title="Ver Dados do Cliente">
...
function editar(id, cp1, cp2, cp3, cp4, cp5, cp6, cp7, cp8, cp9, cp10){
	...
	$('#<?=$campo8?>').val(cp8);
	$('#target').attr('src','config/img/'+'produtos'+'/'+cp9);
	$('#<?=$campo10?>').val(cp10);
...
function limparCampos(){
	...
	$('#<?=$campo6?>').val('');
	$('#<?=$campo9?>').val('');
	$('#target').attr('src','config/img/'+'produtos'+'/sem-foto.jpg');
	$('#quantidade').val('');
	$('#mensagem').text('');
...
function mostrarDados(id, cp1, cp2, cp3, cp4, cp5, cp6, cp7, cp8, cp9, cp10){
	...
	$('#campo7').text(cp7);
	$('#campo8').text(cp8);
	$('#imagem_dados').attr('src','config/img/'+'produtos'+'/'+cp9);
	$('#campo10').text(cp10);
