...
class CrudLancaDespesa extends Connection
{
    ...
    if($id == ""){
        ...
        data_recor = curDate(), juros = '0', multa = '0', desconto = '0',  jurosporc = :campo16, multaporc = :campo15, descontoporc = :campo14");
            ...
            $query = $pdo->prepare("UPDATE $pagina set descricao = :campo1, cliente = :campo2, saida = :campo3, documento = :campo4, plano_conta = :campo5, data_emissao = :campo6, vencimento = :campo7, frequencia = :campo8, valor = :campo9, usuario_lanc = :campo10, 
            jurosporc = :campo16, multaporc = :campo15, descontoporc = :campo14 WHERE id = '$id'");
    ...
    $query->bindValue(":campo16", "00");
    $query->bindValue(":campo15", "00");
    $query->bindValue(":campo14", "00");
    ...
    if($alterou_data == 'Sim'){
        ...
        $query = $pdo->query("SELECT * from $pagina where (vencimento >= '$dataInicial' and vencimento <= '$dataFinal') and status LIKE '$status'  order by id ");
        }
        
    }else if($status != '%%' and $alterou_data == ''){
        $query = $pdo->query("SELECT * from $pagina where status LIKE '$status'  order by vencimento ");
    }
    
    else if($vencidas == 'Vencidas'){
        $query = $pdo->query("SELECT * from $pagina where vencimento < curDate()  order by vencimento  ");
    }
        ...
        $query = $pdo->query("SELECT * from $pagina where vencimento = '$data_amanha'  order by vencimento  ");
    }
    
    else{
        $query = $pdo->query("SELECT * from $pagina where status = 'Pendente' order by vencimento  ");
    ...
    $query = $pdo->prepare("SELECT * from $pagina where id = :id ");
    ...
    if($status_conta == 'Paga'){
        echo 'Esta despesa ja esta paga';
        exit();
    }
    // Consulta preparada: fornecedor pode vir do cadastro da despesa.
    ...
    // Transacao protege despesa, movimentacao e saldo contra gravacao parcial.
    try {
    ...
    if($valor == $cp9){
        ...
        $pdo->query("UPDATE $pagina set descricao = '$cp1', saida = '$saida', usuario_baixa = '$id_usuario', status = 'Pendente', juros = '00', multa = '00', desconto = '00', valor = '$cp9', subtotal = '$subtotal', data_baixa = curDate() where id = '$id'");
    ...
    $pdo->query("INSERT INTO movimentacoes set tipo = 'Despesas', S = '$subtotal', movimento = 'Despesas', descricao = '$descricao_conta', valor = '$subtotal', usuario = '$id_usuario', data = curDate(), lancamento = '$cp3', 
    plano_conta = '$cp5', documento = '$cp4', caixa_periodo = '$caixa_aberto', conta_pag = '$id'");
    ...
    $pdo->query("INSERT INTO movimentacoes set tipo = 'Despesas', S = '$subtotal', movimento = 'Despesas', descricao = '$descricao_conta', valor = '$subtotal', usuario = '$id_usuario', data = curDate(), lancamento = '$cp3', 
    plano_conta = '$cp5', documento = '$cp4', caixa_periodo = '$idbanc', conta_pag = '$id'");
    $idMovimento = $pdo->lastInsertId();
    ...
    usuario = '$usuaMov', tipo = 'Despesas', data = '$dataMov', plano_conta = '$descMov', valor = '$valorMov', 
    ...
    echo 'Baixado com Sucesso';
        ...
        $pdo->query("UPDATE $pagina set saida = '$cp3', usuario_baixa = '$id_usuario', status = 'Pendente', juros = '00', multa = '00', desconto = '00', subtotal = '$subtotal', data_baixa = curDate() where id = '$id'");
     
    }
    
    // Auditoria financeira: registra a baixa e o saldo restante da despesa.
    $this->registrarAuditoriaFinanceira($pdo, 'baixa', $pagina, $id_conta_baixa, $id_usuario, $valor_original_baixa, $cp9, 'Baixa de despesa');
    ...
    while($i <= $qtd_parcelas){
        ...
        $pdo->query("INSERT INTO $pagina set descricao = '$nova_descricao', cliente = '$cp2', saida = '$cp3', 
                documento = '$cp4', plano_conta = '$cp5', data_emissao = curDate(), vencimento = '$novo_vencimento', 
